<template>
	<view >
		...
		<search-com-page title="直入单管理" subtitle="直入单查询" placeholder="直入单号" @searchChange="searchChange"
			...
			<view slot="search">
				<u-form :model="form" ref="uForm" :border-bottom="false" label-position="top" :error-type="['border']">
					<!-- <u-form-item label="直入直出单号" prop="name" :border-bottom="false">
					<u-input border height="95" v-model="form.name" placeholder="请输入直入直出单号" :clearable="false" />
				</u-form-item> -->
		...
		<z-card class="mt-24 list" v-for="(item, index) in list" :key="index">
			<z-more>
				<view slot="top" class="comm-detail-text">
					<view>直入直出单号：</view>
					<view class="right">{{item.inOutputNo}}</view>
					<view>专业名称：</view>
				...
				<view slot="bottom" class="comm-detail-text">
					...
					<view>领料人：</view>
					<view class="right">{{item.pickUser}}</view>
					<view>审核人：</view>
		...
		methods: {
			...
			getDataList() {
				this.$axios.post('/inOutput/inOutputList', {
					...
					"inOutputType": this.current == 0 ? 2 : 1,
			...
			manageData(id) {
				uni.navigateTo({
					url: '../manage/manage?id=' + id+'&state='+(this.current==0?2:1),
					success: res => {},
					fail: () => {},
					complete: () => {}
				});
			...
			//删除数据
			deleteData() {
				this.$axios.post('/inOutput/deleteByIds', this.id).then(res => {
